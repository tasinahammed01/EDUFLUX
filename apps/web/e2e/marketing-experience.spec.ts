...
test("marketing experience stays stable across motion tiers and navigation",async({page})=>{
 ...
 await page.setViewportSize({width:1440,height:900});await page.goto("/");await page.getByRole("link",{name:"Sign in"}).first().click();await expect(page).toHaveURL(/\/login$/);await page.goBack();await expect(page.locator("canvas")).toHaveCount(1);await page.evaluate(()=>window.scrollTo(0,0));await page.getByRole("link",{name:"Start teaching"}).first().click();await expect(page).toHaveURL(/\/register$/);await page.goBack();await expect(page.locator("canvas")).toHaveCount(1);expect(errors).toEqual([]);
});
test("reduced motion keeps critical content visible",async({page})=>{await page.emulateMedia({reducedMotion:"reduce"});await page.goto("/");await expect(page.getByRole("heading",{name:/Teach with clarity/})).toBeVisible();await expect(page.getByText(/Feedback shouldn/)).toBeAttached();await expect(page.getByRole("link",{name:"Start teaching"}).first()).toBeVisible()});
