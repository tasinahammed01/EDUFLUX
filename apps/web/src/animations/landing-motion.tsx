...
export function LandingMotion({ children }: { children: React.ReactNode }) {
  ...
  useGSAP(
    () => {
      ...
      mm.add("(min-width: 1200px)", () => {
        const hero = createHeroMotion(element),
          sections = createSectionMotion(element),
          micro = createMicroInteractions(element);
        if (hero) cleanups.push(hero);
        cleanups.push(sections, micro);
      });
      mm.add("(min-width: 768px) and (max-width: 1199px)", () => {
        const hero = createHeroMotion(element, 0.72);
        if (hero) cleanups.push(hero);
      });
  ...
  return (
    <div ref={root} className="landing-motion">
      {children}
    </div>
  );
}
