...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  function redirect(session: AuthSession) {
    const next = new URLSearchParams(window.location.search).get("next");
    const safeNext =
      next?.startsWith("/join/") && !next.includes("//") ? next : undefined;
    router.replace(
      session.requiresOnboarding
        ? "/onboarding"
        : (safeNext ??
            (session.user.primaryPersona === "TEACHER"
              ? "/teacher/dashboard"
              : "/student/dashboard")),
    );
    router.refresh();
      ...
      const session =
        mode === "register"
          ? await registerWithEmail(
              parsed.data as typeof input & {
                displayName: string;
                primaryPersona: PrimaryPersona;
              },
            )
          : await loginWithEmail(
              (parsed.data as { email: string; password: string }).email,
              (parsed.data as { email: string; password: string }).password,
            );
      redirect(session);
    } catch (caught) {
  ...
  async function google() {
    ...
    setError("");
    try {
      redirect(await loginWithGoogle());
    } catch (caught) {
      setError(firebaseErrorMessage(caught));
    } finally {
      setPending(false);
  ...
  return (
    <div className="auth-card">
      ...
      <form onSubmit={submit} noValidate>
        ...
        {mode === "register" && (
          <>
            <fieldset className="persona-field">
                ...
                {(["TEACHER", "STUDENT"] as const).map((value) => (
                  ...
                  >
                    ...
                    <span>
                      ...
                      <small>
                        {value === "TEACHER"
                          ? "Create and guide classes"
                          : "Join and learn with a class"}
                      </small>
                    </span>
                  </button>
                ))}
...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M21.6 12.2c0-.7-.1-1.4-.2-2H12v3.9h5.4a4.6 4.6 0 01-2 3v2.5h3.2c1.9-1.7 3-4.3 3-7.4"
      />
      <path
        fill="#34A853"
        d="M12 22c2.7 0 5-.9 6.6-2.4l-3.2-2.5c-.9.6-2 1-3.4 1-2.6 0-4.8-1.8-5.6-4.2H3.1v2.6A10 10 0 0012 22"
      />
      <path
        fill="#FBBC05"
        d="M6.4 13.9A6 6 0 016.1 12c0-.7.1-1.3.3-1.9V7.5H3.1A10 10 0 002 12c0 1.6.4 3.1 1.1 4.5z"
      />
      <path
        fill="#EA4335"
        d="M12 5.9c1.5 0 2.8.5 3.8 1.5l2.9-2.8A9.7 9.7 0 0012 2a10 10 0 00-8.9 5.5l3.3 2.6c.8-2.4 3-4.2 5.6-4.2"
      />
    </svg>
  );
}
function Field({
  ...
  return (
    <label className="form-field">
      ...
      />
      {description && !error && (
        <small id={`${name}-description`}>{description}</small>
      )}
      {error && (
