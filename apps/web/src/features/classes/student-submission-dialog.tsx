...
import { CheckCircle2, FileText, Image as ImageIcon, LoaderCircle, UploadCloud, X } from "lucide-react";
  ...
  return <div className="submission-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !locked) onClose(); }}>
    <div ref={dialogRef} className="student-submission-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId}>
      ...
      {loading ? <div className="submission-dialog-loading"><LoaderCircle className="spinner" /> Loading your draft…</div> : submission && <>
        ...
        {(submission.draftFiles.length > 0 || files.length > 0) && <div className="submission-attachments"><h3>Attachments</h3>{submission.draftFiles.map((file) => <div key={file.id}><FileText /><span title={file.originalName}><strong>{file.originalName}</strong><small>{formatBytes(file.sizeBytes)} · Ready</small></span><CheckCircle2 className="file-ready" /><button type="button" aria-label={`Remove ${file.originalName}`} disabled={locked || saving} onClick={() => void removeReadyFile(file.id)}><X /></button></div>)}{files.map((item) => <div key={item.id}>{item.previewUrl ? <Image src={item.previewUrl} alt="" width={40} height={40} unoptimized /> : <FileText />}<span title={item.file.name}><strong>{item.file.name}</strong><small>{formatBytes(item.file.size)} · Ready to upload</small></span><button type="button" aria-label={`Remove ${item.file.name}`} disabled={locked} onClick={() => setFiles((current) => current.filter((file) => file.id !== item.id))}><X /></button></div>)}</div>}
